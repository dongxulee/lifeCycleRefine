...
####################################################################################### high skill feature
detEarning = jnp.concatenate([detEarning[:46], detEarning[46:]-35])
...
Dn[-1] = 1
Dn = jnp.array(Dn)
# at age of 65 how much money is in the 401k account for both rich and poor
n_rich = 200
n_poor = 100
...
'''
# actions dicretization(consumption, stock, action)
numGrid = 20
As = np.array(np.meshgrid([1], [0,1], [0,1])).T.reshape(-1,3)
As = jnp.array(As)
